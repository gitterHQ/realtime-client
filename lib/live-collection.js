...
module.exports = Backbone.Collection.extend({
  ...
  listen: function(options) {
    ...
    console.log('CALLING subscribe', url);
    this.subscription = this.client.subscribe(url, function(message) {
      self._onDataChange(message);
    });

    if (options && options.snapshot) {
      this.client.registerSnapshotHandler(url, this);
    }

    this.subscription.errback(function(error) {
      log.error('coll: Subscription error for ' + url, error);
    });
  },
